...
// Original path: rust/src/a64/encode.rs.  License: MIT (see LICENSE-JASM in this
...
fn sf_of(r: Reg) -> Result<u32> {
    match r.class {
        ...
        RegClass::W => Ok(0),
        _ => bail!(
            "expected a general-purpose register here, got {:?}",
            r.class
        ),
    }
}
...
pub fn encode(mnemonic: &str, ops: &[Operand]) -> Result<Encoded> {
    ...
    if let Some(suffix) = mnemonic.strip_prefix("b.") {
        let cond =
            cond_code(suffix).ok_or_else(|| anyhow::anyhow!("unknown condition `{suffix}`"))?;
        let [Sym(t)] = ops else {
            bail!("b.{suffix} expects a label")
        };
        // 0x54000000 | imm19<<5 | cond ; imm19 filled by the driver.
        return Ok(Encoded::word_fixup(
            0x5400_0000 | cond,
            FixupKind::Branch19,
            t,
        ));
    ...
    match (mnemonic, ops) {
        ...
        ("ret", [Reg(r)]) => Ok(Encoded::word(
            0xD65F_0000 | (want(*r, RegClass::X, "x")? << 5),
        )),
        ...
        ("adrp", [Reg(d), Sym(s)]) => {
            ...
            Ok(Encoded::word_fixup(
                0x9000_0000 | rn(*d),
                FixupKind::AdrpPage21,
                target,
            ))
        }
        ...
        ("br", [Reg(r)]) => Ok(Encoded::word(
            0xD61F_0000 | (want(*r, RegClass::X, "x")? << 5),
        )),
        ("blr", [Reg(r)]) => Ok(Encoded::word(
            0xD63F_0000 | (want(*r, RegClass::X, "x")? << 5),
        )),
        ("cbz", [Reg(r), Sym(t)]) => compare_branch(*r, t, false),
        ("cbnz", [Reg(r), Sym(t)]) => compare_branch(*r, t, true),

        _ => bail!(
            "unsupported AArch64 instruction `{mnemonic}` with {} operand(s)",
            ops.len()
        ),
    }
}
...
fn mov_reg(d: Reg, s: Reg) -> Result<Encoded> {
    ...
    let base = if d.class == RegClass::X {
        0xAA00_03E0
    } else {
        0x2A00_03E0
    };
...
fn move_wide(d: Reg, imm: &Operand, kind: MoveWide) -> Result<Encoded> {
    ...
    let w =
        (sf << 31) | (opc << 29) | (0b100101 << 23) | (hw << 21) | ((imm16 as u32) << 5) | rn(d);
    Ok(Encoded::word(w))
}
...
fn logical_imm(opc: u32, d: Reg, n: Reg, imm: i64) -> Result<Encoded> {
    ...
    let (nbit, immr, imms) = encode_bitmask(imm as u64, reg_size).ok_or_else(|| {
        anyhow::anyhow!("{imm:#x} is not a valid AArch64 logical (bitmask) immediate")
    })?;
    let w = (sf << 31)
        | (opc << 29)
        | (0b100100 << 23)
...
fn data3(d: Reg, n: Reg, m: Reg, a: Reg, sub: bool) -> Result<Encoded> {
    ...
    Ok(Encoded::word(
        base | o0 | (rn(m) << 16) | (rn(a) << 10) | (rn(n) << 5) | rn(d),
    ))
}
...
fn data3_long(d: Reg, n: Reg, m: Reg, signed: bool, sub: bool) -> Result<Encoded> {
    ...
    let o0 = if sub { 0x8000 } else { 0 };
    Ok(Encoded::word(
        base | o0 | (rn(m) << 16) | (31 << 10) | (rn(n) << 5) | rn(d),
    ))
}
...
fn shift_var(d: Reg, n: Reg, m: Reg, op2: u32) -> Result<Encoded> {
    ...
    Ok(Encoded::word(
        base | (op2 << 10) | (rn(m) << 16) | (rn(n) << 5) | rn(d),
    ))
}
...
fn dp1(d: Reg, n: Reg, opc2: u32) -> Result<Encoded> {
    let sf = sf_of(d)?;
    Ok(Encoded::word(
        (sf << 31) | 0x5AC0_0000 | (opc2 << 10) | (rn(n) << 5) | rn(d),
    ))
}
...
fn try_movi(mnem: &str, ops: &[Operand]) -> Option<Result<Encoded>> {
    ...
    let word = |q: u32, op: u32, cmode: u32, imm8: u8, d: u32| {
        ...
        Encoded::word(
            0x0F00_0400 | (q << 30) | (op << 29) | (abc << 16) | (cmode << 12) | (defgh << 5) | d,
        )
    };
    ...
    let [VReg { num: d, arr }, immop] = ops else {
        return None;
    };
    ...
    Some((|| {
        ...
        let (op, cmode) = match (mnem, esize) {
            ...
            ("bic" | "orr", 2) => {
                ...
                (
                    (mnem == "bic") as u32,
                    0b0001 | (lsl_x(lsl.unwrap_or(0))? << 1),
                )
            }
...
fn try_simd(mnem: &str, ops: &[Operand]) -> Option<Result<Encoded>> {
    ...
    // ── by-element: Vd.<arr>, Vn.<arr>, Vm.<ts>[index] ──────────────────────
    {
        ...
        if let Some(base) = simd_byelem(base_mnem) {
            if let [VReg { num: d, arr: _ }, VReg { num: n, arr: an }, VElem {
                num: m,
                esize,
                index,
            }] = ops
        ...
        let (list, mem, rm_post) = match ops {
            ...
            _ => {
                return Some(Err(anyhow::anyhow!(
                    "structured ld/st expects `{{list}}, [Xn]{{, post}}`"
                )))
            }
        };
        let Operand::VList { first, count, arr } = list else {
            unreachable!()
        };
        return Some((|| {
    ...
    if mnem == "tbl" || mnem == "tbx" {
        if let [VReg { num: d, arr: ad }, Operand::VList {
            first,
            count,
            arr: _,
        }, VReg { num: m, arr: am }] = ops
        {
            return Some((|| {
                if ad != am {
    ...
    if mnem == "ext" {
        if let [VReg { num: d, arr: ad }, VReg { num: n, arr: an }, VReg { num: m, arr: am }, Operand::Imm(idx)] =
            ops
        {
            if ad != an || ad != am {
    ...
    if mnem == "umov" || mnem == "smov" || mnem == "mov" {
        ...
        }] = ops
        {
            let to_x = match g.class {
                ...
                _ => {
                    return Some(Err(anyhow::anyhow!(
                        "umov/smov destination must be a GP register"
                    )))
                }
            };
            let sz = elem_size(*esize);
            let imm5 = ((*index as u32) << (sz + 1)) | (1 << sz);
            let base = if mnem == "smov" {
                0x0E00_2C00
            } else {
                0x0E00_3C00
            };
    ...
    if let Some((base, left)) = simd_shift_imm(mnem) {
        if let [VReg { num: d, arr: ad }, VReg { num: n, arr: an }, Operand::Imm(sh)] = ops {
            ...
            return Some(Ok(Encoded::word(
                base | (q << 30) | (immhb << 16) | ((*n as u32) << 5) | *d as u32,
            )));
        }
    }
    ...
    if let [VReg { num: d, arr: ad }, VReg { num: n, arr: an }, Operand::FpImm(z)] = ops {
        if let Some(base) = simd2_fcmp_zero_base(mnem) {
            ...
            return Some(Ok(Encoded::word(
                base | (q << 30) | (sz << 22) | ((*n as u32) << 5) | *d as u32,
            )));
        }
    }
    ...
    // conveyed by that arrangement (.16b ⇒ Q=1), so we strip it.
    {
        ...
        if let Some((base, from)) = simd_widen(base_mnem) {
            ...
            } else if let [VReg { num: d, arr: ad }, VReg { num: n, arr: an }, VReg { num: m, arr: am }] =
                ops
            {
                let src = match from {
                    QFrom::Vn => *an,
    ...
    // ── fcvtn/fcvtxn (narrow) + fcvtl (long) ────────────────────────────────
    {
        ...
        if base_mnem == "fcvtl" {
            if let [VReg { num: d, arr: ad }, VReg { num: n, arr: an }] = ops {
                ...
                return Some(Ok(Encoded::word(
                    0x0E21_7800 | (q << 30) | (sz << 22) | ((*n as u32) << 5) | *d as u32,
                )));
            }
        }
    }
    ...
    if let [Reg(d), VReg { num: n, arr: an }] = ops {
        if let Some(base) = simd_across_base(mnem) {
            ...
            return Some(Ok(Encoded::word(
                base | (q << 30) | (sz << 22) | ((*n as u32) << 5) | d.num as u32,
            )));
        }
    }
    ...
    if mnem == "dup" {
        if let [VReg { num: d, arr: ad }, VElem {
            num: n,
            esize,
            index,
        }] = ops
        ...
        if let [VReg { num: d, arr: ad }, Reg(g)] = ops {
            if !matches!(g.class, RegClass::X | RegClass::W) {
                return Some(Err(anyhow::anyhow!(
                    "dup source must be a GP register or vector element"
                )));
            }
            let sz = arr_esize(*ad);
            let imm5 = 1u32 << sz;
...
fn try_crypto(mnem: &str, ops: &[Operand]) -> Option<Result<Encoded>> {
    ...
    if sha3 != 0 {
        let [a, b, c] = ops else {
            return Some(Err(anyhow::anyhow!(
                "{mnem} expects three register operands"
            )));
        };
...
fn try_fp(mnem: &str, ops: &[Operand]) -> Option<Result<Encoded>> {
    ...
    let two = |opc: u32| -> Option<Result<Encoded>> {
        let [Reg(d), Reg(n), Reg(m)] = ops else {
            return None;
        };
        let ft = ftype_of(*d)?;
        if d.class != n.class || d.class != m.class {
    ...
    let three = |base_s: u32| -> Option<Result<Encoded>> {
        let [Reg(d), Reg(n), Reg(m), Reg(a)] = ops else {
            return None;
        };
        let ft = ftype_of(*d)?;
        Some(Ok(Encoded::word(
    ...
    if mnem == "fcmp" || mnem == "fcmpe" {
        ...
        return Some((|| match ops {
            [Reg(n), Reg(m)] => {
                let ft = ftype_of(*n).context("fcmp needs FP operands")?;
                Ok(Encoded::word(
                    0x1E20_2000 | (ft << 22) | (rn(*m) << 16) | (rn(*n) << 5) | e,
                ))
            }
            [Reg(n), Operand::FpImm(z)] if *z == 0.0 => {
                let ft = ftype_of(*n).context("fcmp needs FP operands")?;
                Ok(Encoded::word(
                    0x1E20_2000 | (ft << 22) | (rn(*n) << 5) | e | 0x08,
                ))
            }
    ...
    if mnem == "fccmp" || mnem == "fccmpe" {
        ...
        return Some(Err(anyhow::anyhow!(
            "fccmp: expected `Sn, Sm, #nzcv, cond`"
        )));
    }
        ...
        return Some((|| {
            match ops {
                ...
                [Reg(d), Reg(n)] if ftype_of(*d).is_some() && d.class == n.class => {
                    ...
                    Ok(Encoded::word(
                        0x1E20_4000 | (ft << 22) | (rn(*n) << 5) | rn(*d),
                    ))
                }
                // GP → FP (opcode 111).
                [Reg(d), Reg(n)]
                    if ftype_of(*d).is_some() && matches!(n.class, RegClass::X | RegClass::W) =>
                {
                    let ft = ftype_of(*d).unwrap();
                    let sf = sf_of(*n)?;
                    Ok(Encoded::word(
                        (sf << 31) | 0x1E20_0000 | (ft << 22) | (7 << 16) | (rn(*n) << 5) | rn(*d),
                    ))
                }
                // FP → GP (opcode 110).
                [Reg(d), Reg(n)]
                    if matches!(d.class, RegClass::X | RegClass::W) && ftype_of(*n).is_some() =>
                {
                    let ft = ftype_of(*n).unwrap();
                    let sf = sf_of(*d)?;
                    Ok(Encoded::word(
                        (sf << 31) | 0x1E20_0000 | (ft << 22) | (6 << 16) | (rn(*n) << 5) | rn(*d),
                    ))
                }
                // FP immediate.
                [Reg(d), Operand::FpImm(v)] if ftype_of(*d).is_some() => {
                    let ft = ftype_of(*d).unwrap();
                    let imm8 =
                        fp_imm8(*v).with_context(|| format!("{v} is not an 8-bit FP immediate"))?;
                    Ok(Encoded::word(
                        0x1E20_1000 | (ft << 22) | ((imm8 as u32) << 13) | rn(*d),
                    ))
                }
...
fn csel(d: Reg, n: Reg, m: Reg, cond: u32, kind: CSel) -> Result<Encoded> {
    ...
    Ok(Encoded::word(
        base | (rn(m) << 16) | (cond << 12) | (rn(n) << 5) | rn(d),
    ))
}
...
fn try_intmisc(mnem: &str, ops: &[Operand]) -> Option<Result<Encoded>> {
    ...
    let adc_base =
        |sf: u32, op: u32, s: u32| (sf << 31) | (op << 30) | (s << 29) | (0b11010000 << 21);
    ...
    if let Some((op, s)) = carry {
        if let [Reg(d), Reg(n), Reg(m)] = ops {
            return Some((|| {
                let sf = sf_of(*d)?;
                Ok(Encoded::word(
                    adc_base(sf, op, s) | (rn(*m) << 16) | (rn(*n) << 5) | rn(*d),
                ))
            })());
        }
    }
    if mnem == "ngc" || mnem == "ngcs" {
        ...
        if let [Reg(d), Reg(m)] = ops {
            return Some((|| {
                let sf = sf_of(*d)?;
                Ok(Encoded::word(
                    adc_base(sf, 1, s) | (rn(*m) << 16) | (31 << 5) | rn(*d),
                ))
            })());
        }
    }
...
fn try_system(mnem: &str, ops: &[Operand]) -> Option<Result<Encoded>> {
    ...
    if mnem == "msr" {
        ...
        if let [Sym(reg), Reg(t)] = ops {
            return Some((|| {
                let enc =
                    sysreg(reg).with_context(|| format!("unknown system register `{reg}`"))?;
...
mod tests {
    ...
    fn enc(asm: &str) -> Vec<u8> {
        let Line::Insn { mnemonic, ops } = parse_line(asm).unwrap() else {
            panic!("not an insn: {asm}")
        };
        encode(&mnemonic, &ops)
            .unwrap_or_else(|e| panic!("encode `{asm}`: {e}"))
            .bytes
    }
