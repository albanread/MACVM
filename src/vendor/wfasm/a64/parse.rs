...
// Original path: rust/src/a64/parse.rs.  License: MIT (see LICENSE-JASM in this
...
pub fn parse_line(raw: &str) -> Result<Line> {
    ...
    let ops = parse_operands(rest).with_context(|| format!("operands of `{raw}`"))?;
    ...
    for chunk in chunks {
        ...
        if let Operand::Imm(v) = op {
            if let Some(Operand::Mem(m)) = ops.last() {
                if m.addr == Addr::Base {
                    let base = m.base;
                    *ops.last_mut().unwrap() = Operand::Mem(Mem {
                        base,
                        addr: Addr::PostIndex(v),
                    });
                    continue;
                }
            }
...
fn parse_shift_modifier(chunk: &str) -> Result<Option<(Shift, u32)>> {
    ...
    let amount: u32 = amt
        .parse()
        .with_context(|| format!("shift amount in `{chunk}`"))?;
    Ok(Some((kind, amount)))
}
...
fn parse_msl(chunk: &str) -> Result<Option<u32>> {
    ...
    Ok(Some(
        amt.parse()
            .with_context(|| format!("msl amount in `{chunk}`"))?,
    ))
}
...
fn parse_operand(s: &str) -> Result<Operand> {
    ...
    if let Some(imm) = s.strip_prefix('#') {
        ...
        if is_float {
            return Ok(Operand::FpImm(
                t.parse::<f64>()
                    .with_context(|| format!("fp immediate `{t}`"))?,
            ));
        }
    ...
    match it.next() {
        ...
        Some(ext_spec) => {
            ...
            let (amount, scaled) = if rest.is_empty() {
                ...
                (
                    n.parse()
                        .with_context(|| format!("extend amount `{rest}`"))?,
                    true,
                )
            };
            Ok(Addr::RegOffset {
...
pub fn parse_reg(tok: &str) -> Option<Reg> {
    ...
    let max = if matches!(class, RegClass::X | RegClass::W) {
        30
    } else {
        31
    };
...
mod tests {
    ...
    #[test]
    fn regs_and_imm() {
        let Line::Insn { mnemonic, ops } = parse_line("add x0, x1, x2").unwrap() else {
            panic!()
        };
    ...
    #[test]
    fn sp_and_imm_shift() {
        let Line::Insn { ops, .. } = parse_line("sub sp, sp, #16").unwrap() else {
            panic!()
        };
        assert_eq!(
            ops[0],
        ...
        let Line::Insn { ops, .. } = parse_line("movk x0, #0xabcd, lsl #32").unwrap() else {
            panic!()
        };
    ...
    #[test]
    fn memory_forms() {
        let Line::Insn { ops, .. } = parse_line("ldr x0, [x1, #8]").unwrap() else {
            panic!()
        };
        assert_eq!(
            ops[1],
        ...
        let Line::Insn { ops, .. } = parse_line("str x0, [sp, #-16]!").unwrap() else {
            panic!()
        };
        assert_eq!(
            ops[1],
    ...
    #[test]
    fn shifted_register() {
        let Line::Insn { ops, .. } = parse_line("add x0, x1, x2, lsl #3").unwrap() else {
            panic!()
        };
        assert_eq!(
            ops[2],
    ...
    fn fp_and_simd_operands() {
        let Line::Insn { ops, .. } = parse_line("fadd d0, d1, d2").unwrap() else {
            panic!()
        };
        assert_eq!(
            ops[0],
        ...
        let Line::Insn { ops, .. } = parse_line("fmov d0, #1.0").unwrap() else {
            panic!()
        };
        assert_eq!(ops[1], Operand::FpImm(1.0));

        let Line::Insn { ops, .. } = parse_line("add v0.4s, v1.4s, v2.4s").unwrap() else {
            panic!()
        };
        assert_eq!(
            ops[0],
        ...
        let Line::Insn { ops, .. } = parse_line("dup v0.2d, v1.d[1]").unwrap() else {
            panic!()
        };
        assert_eq!(
            ops[0],
        ...
        let Line::Insn { ops, .. } = parse_line("ldr q3, [x0, #16]").unwrap() else {
            panic!()
        };
        assert_eq!(
            ops[0],
    ...
    fn label_and_branch() {
        ...
        let Line::Insn { mnemonic, ops } = parse_line("b.eq done").unwrap() else {
            panic!()
        };
