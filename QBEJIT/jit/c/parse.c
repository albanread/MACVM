...
/* Linkage-only change from upstream: was `static`. ir_builder.c (a new,
 * non-upstream file — see its own header comment) reuses this exact
 * function to validate functions it constructs directly, the same
 * validation parse() already runs on every text-parsed function. No
 * behavior change to the text path; see ir_builder.h's module doc for
 * the full rationale. Declared in all.h's "parse.c" section. */
void
typecheck(Fn *fn)
{
	Blk *b;
